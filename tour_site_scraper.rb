...
# profile = Selenium::WebDriver::Firefox::Options.new(args: ['-headless'])
...
ticket_search_date_from.upto(ticket_search_date_to) do |dt|
  ...
  driver.navigate.to 'https://www.tour.ne.jp/j_air/list/?adult=1&air_type=2&arr_out=ITM&change_date_in=0&change_date_out=0&date_out='+ departure_date + '&dpt_out=TYO&time_from_out=0600&time_to_out=0700&time_type_out=0'
  # sleep(1)  # Wait 1s to load the page properly

  # MAX_RETRY = 100
  begin
    retries ||= 0
    ticket_summary_button = nil
  ...
  wait.until { driver.find_element(class: "toggle-btn-company").displayed? }
  wait.until { driver.find_element(class: "airline-name").displayed? }
